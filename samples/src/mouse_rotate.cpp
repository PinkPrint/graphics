...
//	V = glm::lookAt(glm::vec3(1.0f, 1.0f, 1.0f)*5.0f, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void mouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:
		if(state == GLUT_DOWN)
		{
			...
			camera.position_dragging[0] = x;
			camera.position_dragging[1] = y;
		}
		else	camera.dragging = false;
		break;
	}
}
...
void motion(int x, int y)
{
	if(camera.dragging)
	{
		glm::vec4	dir(x - camera.position_dragging[0], -(y - camera.position_dragging[1]), 0.0f, 0.0f);
		camera.R = glm::rotate(glm::mat4(1.0f), 1.0f*GLfloat(dir.length()), glm::vec3(-dir.y, dir.x, 0.0f))*camera.R;
		camera.position_dragging[0] = x;
		camera.position_dragging[1] = y;
		glutPostRedisplay();
	}
