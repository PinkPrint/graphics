...
void init(void)
{
	...
	glVertexAttribPointer(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, 0, BUFFER_OFFSET(0));
	glVertexAttribDivisor(1, 2);
	glEnableVertexAttribArray(1);

	glGenBuffers(1, &vbo_offset_y);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_offset_y);
	glBufferData(GL_ARRAY_BUFFER, sizeof(offsets_y), offsets_y, GL_STATIC_DRAW);
	glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
	glVertexAttribDivisor(2, 1);
	glEnableVertexAttribArray(2);

	GLuint h_prog = build_program_from_files("instanced_vert_attribs.vert", "instanced_vert_attribs.frag");
