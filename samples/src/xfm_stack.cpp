...
void display(void)
{
	...
	mstack.push();
	mstack.load_identity();

	glBindVertexArray(vao_quad);

#define	WIDTH_RED	0.4f
#define	LENGTH_RED	1.0f
	glm::mat4	Sr = glm::scale(glm::mat4(1.0f), glm::vec3(LENGTH_RED, WIDTH_RED, 1.0f));
	glm::mat4	Tr_low = glm::translate(glm::mat4(1.0f), glm::vec3((LENGTH_RED - WIDTH_RED)/2.0f, 0.0f, 0.0f));
	glm::mat4	Rr = glm::rotate(glm::mat4(1.0f), angle_R, glm::vec3(0.0f, 0.0f, 1.0f));
	mstack.mult_matrix(P);
	mstack.mult_matrix(T);
	mstack.push();
		mstack.mult_matrix(T_base);
		mstack.mult_matrix(Rr);
		mstack.push();
			mstack.mult_matrix(Tr_low);
			mstack.mult_matrix(Sr);
			glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm::value_ptr(mstack.top()));
...
#define	LENGTH_GREEN	0.8f
	...
	glm::mat4	Rg = glm::rotate(glm::mat4(1.0f), angle_G, glm::vec3(0.0f, 0.0f, 1.0f));
		mstack.mult_matrix(Tr_high);
		mstack.mult_matrix(Rg);
		mstack.push();
			mstack.mult_matrix(Tg_low);
			mstack.mult_matrix(Sg);
//			glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm::value_ptr(mstack.top()));
		mstack.pop();
...
#define	LENGTH_BLUE	0.3f
	...
	glUniform4f(loc_color, 0, 0, 1, 1);
		mstack.push();
			mstack.mult_matrix(Tg_high1);
			mstack.mult_matrix(Rb1);
			mstack.mult_matrix(Tb);
			mstack.mult_matrix(Sb);
//	glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm::value_ptr(P*T*T_base*Rr*Tr_high*Rg*Tg_high1*Rb1*Tb*Sb));
			glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm::value_ptr(mstack.top()));
		mstack.pop();
	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
		mstack.push();
			mstack.mult_matrix(Tg_high2);
			mstack.mult_matrix(Rb2);
			mstack.mult_matrix(Tb);
			mstack.mult_matrix(Sb);
			glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm::value_ptr(mstack.top()));
		mstack.pop();
