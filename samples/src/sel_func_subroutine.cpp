...
void init(void)
{
	...
	h_prog = build_program_from_files("triangles.vert", "sel_func_subroutine.frag");

	glUseProgram(h_prog);

	loc_func = glGetSubroutineUniformLocation(h_prog, GL_FRAGMENT_SHADER, "func");
	idx_shader_func[0] = glGetSubroutineIndex(h_prog, GL_FRAGMENT_SHADER, "func_first");
	idx_shader_func[1] = glGetSubroutineIndex(h_prog, GL_FRAGMENT_SHADER, "func_second");

	printf("location of the uniform subroutine \"func\" = %d\n", loc_func);
	printf("index of the subroutine \"func_first\" = %d\n", idx_shader_func[0]);
	printf("index of the subroutine \"func_second\" = %d\n", idx_shader_func[1]);
