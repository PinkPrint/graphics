...
void init(void)
{
	...
	h_prog = build_program_from_files("triangles.vert", "sel_func_uniform_var.frag");
...
void display(void)
{
	...
	glUseProgram(h_prog);
	glUniform1i(glGetUniformLocation(h_prog, "idx_func"), idx_func);
	glBindVertexArray(vao);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	{
		case ' ':
			idx_func = (idx_func+1)&0x01;	// toggles between 0 & 1
