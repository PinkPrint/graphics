...
void display(void)
{
	...
	glUniformMatrix4fv(glGetUniformLocation(h_prog, "MVP"), 1, GL_FALSE, glm::value_ptr(P*MV));
	glBindVertexArray(vao_tris);
...
void refresh_proj(void)
{
	GLfloat	ratio = GLfloat(width)/GLfloat(height);
	if(proj_ortho)
	{
		if(width > height)
			P = glm::ortho(-2.0f*ratio, 2.0f*ratio, -2.0f, 2.0f, 0.0f, 4.0f);
		else
			P = glm::ortho(-2.0f, 2.0f, -2.0f/ratio, 2.0f/ratio, 0.0f, 4.0f);
	}
	else
	{
		if(width > height)
			P = glm::frustum(-1.0f*ratio, 1.0f*ratio, -1.0f, 1.0f, 1.0f, 3.0f);
		else
			P = glm::frustum(-1.0f, 1.0f, -1.0f/ratio, 1.0f/ratio, 1.0f, 3.0f);
	}
}
