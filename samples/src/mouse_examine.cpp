...
public:
	glm::mat4	T;
	glm::mat4	R;
	SceneGL() {T = R = glm::mat4(1.0f); root = NULL;}
...
//	V = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -5.0f));
//	V = V*glm::rotate(glm::mat4(1.0f), 90.0f, glm::vec3(0.0f, 0.0f, 1.0f));
	V = glm::lookAt(glm::vec3(1.0f, 1.0f, 1.0f)*3.0f, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void display(void)
{
	P = glm::perspective(camera.fovy, 1.0f, 0.1f, 10.0f);
...
void motion(int x, int y)
{
	...
	case STATE_ROTATING:
		...
		glm::vec3	org_win = glm::project(glm::vec3(0.0f, 0.0f, 0.0f), V*scene.T, P, viewport);
		glm::vec3	dir_win = glm::vec3(gui.offset.y, gui.offset.x, 0.0f);
		glm::vec3	dir_model = glm::unProject(dir_win + org_win, V*scene.T, P, viewport);
            
        // Without the following check, an assertion is generated on OSX (2013/11/29, Minho Kim)
        // Weirdly, dir_model.length() doesn't work...
        if(dir_model.x*dir_model.x + dir_model.y*dir_model.y + dir_model.z*dir_model.z == 0)
            break;
		scene.R = glm::rotate(glm::mat4(1.0f), 1.0f*GLfloat(dir_win.length()), dir_model)*scene.R;
		glutPostRedisplay();
		break;
		}
	case STATE_MOVING:
		...
		glm::vec3	dir_model = glm::unProject(dir_win + org_win, V*scene.T, P, viewport);
		scene.T = glm::translate(glm::mat4(1.0f), dir_model)*scene.T;
		glutPostRedisplay();
		break;
		}
	}
