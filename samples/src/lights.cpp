...
glm::mat4	P, V;
GLuint	h_prog;
enum LightType {LIGHT_TYPE_DIRECTIONAL = 0, LIGHT_TYPE_POSITIONAL, LIGHT_TYPE_SPOT, LIGHT_TYPE_NUM};
const char	*light_type_str[LIGHT_TYPE_NUM] =
{
	"positional light",
	"directional light",
	"spot light",
};
GLuint	light_type = LIGHT_TYPE_DIRECTIONAL;
GLfloat	light_position[4] = {0.0f, 0.0f, 2.0f, 1.0f};
GLfloat	light_spot_cutoff = 180.0f;
...
//#define	FILENAME	"data/models/nurbs.obj"
...
void init(void)
{
	h_prog = build_program_from_files("lights_Blinn_Phong.vert", "lights_Blinn_Phong.frag");
...
void display(void)
{
	...
	glUniform1i(glGetUniformLocation(h_prog, "light.type"), light_type);
	glUniform4fv(glGetUniformLocation(h_prog, "light.position"), 1, light_position);
	...
	glUniform3f(glGetUniformLocation(h_prog, "light.spot.direction"), -light_position[0], -light_position[1], -light_position[2]);
	glUniform1f(glGetUniformLocation(h_prog, "light.spot.cutoff"), cos(DEG2RAD(light_spot_cutoff)));
	glUniform1f(glGetUniformLocation(h_prog, "light.spot.exponent"), 2.0f);
	glUniform1f(glGetUniformLocation(h_prog, "light.attenuation.k0"), 1.0f);
	glUniform1f(glGetUniformLocation(h_prog, "light.attenuation.k1"), 0.0f);
	glUniform1f(glGetUniformLocation(h_prog, "light.attenuation.k2"), 0.0f);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	{
	case ' ':
		light_type = (light_type+1)%LIGHT_TYPE_NUM;
		if(light_type == LIGHT_TYPE_SPOT)	light_spot_cutoff = 10.0f;
		else								light_spot_cutoff = 180.0f;
		if(light_type == LIGHT_TYPE_DIRECTIONAL)	light_position[3] = 0.0f;
		else										light_position[3] = 1.0f;
		printf("light type: %s\n", light_type_str[light_type]);
		printf("spot cut-off = %g\n", light_spot_cutoff);
		break;
	}
}
